        ...
        <div x-show="tab === 'basic'" class="space-y-5">
            <div>
                ...
                <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Full Name</p><p class="mt-0.5 text-sm font-medium text-slate-900">{{ $patient->fullName() }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Gender</p><p class="mt-0.5 text-sm text-slate-900">{{ ucfirst($patient->gender ?? '—') }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Date of Birth</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->date_of_birth ?? '—' }}{{ $age !== null ? " ($age yrs)" : '' }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Phone</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->phone_number ?: '—' }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Email</p><p class="mt-0.5 truncate text-sm text-slate-900">{{ $patient->email ?: '—' }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Address</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->address ?: '—' }}</p></div>
                </div>
            </div>
            <div>
                <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Medical Information</p>
                <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Blood Type</p><p class="mt-0.5 text-sm font-semibold text-red-600">{{ $patient->blood_type ?: '—' }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Allergies</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->allergy ?: '—' }}</p></div>
                </div>
            </div>
            <div>
                <p class="mb-2 text-xs font-semibold uppercase tracking-wider text-slate-400">Emergency Contact</p>
                <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Contact Name</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->emergency_contact_name ?: '—' }}</p></div>
                    <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Contact Phone</p><p class="mt-0.5 text-sm text-slate-900">{{ $patient->emergency_contact_phone ?: '—' }}</p></div>
                </div>
            </div>
            @if($patient->insurance->isNotEmpty())
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                        <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Provider</p><p class="mt-0.5 text-sm text-slate-900">{{ $ins->insurance_provider ?: '—' }}</p></div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Policy Number</p><p class="mt-0.5 text-sm text-slate-900">{{ $ins->policy_number ?: '—' }}</p></div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Coverage</p><p class="mt-0.5 text-sm text-slate-900">{{ $ins->coverage_details ?: '—' }}</p></div>
                        <div class="rounded-lg border border-slate-100 bg-slate-50/60 px-3 py-2"><p class="text-xs text-slate-500">Policy Period</p><p class="mt-0.5 text-sm text-slate-900">{{ $ins->start_date ?? '—' }} &rarr; {{ $ins->end_date ?? '—' }}</p></div>
                    </div>
                </div>
            @endif
